...
const processCommit = async (
  ...
  if (!dirExists) {
    ...
    if (!isGit) {
      ...
      await initializeGit(projectDir);
      gitInitialized = true;
      commitPerformed = await commitChanges(
        projectDir,
...
const setProjectReferenceBranch = async (
  ...
  for (const dir of dirs) {
    try {
      ...
      results.push({
        directory: dir,
        success: true,
        message: `Reference branch preference updated to '${branchName}'.`,
      });
    } catch (error) {
      results.push({ directory: dir, success: false, error: error.message });
    }
  ...
  logger.info(
    `Reference branch preference for project ${projectId} updated to '${branchName}'.`
  );
  return results;
};
