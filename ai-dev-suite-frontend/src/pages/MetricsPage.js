...
import {
    Container, Typography, Paper, Box, Alert, CircularProgress, Card, CardContent,
    Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TableSortLabel, Chip, Divider,
    IconButton, Tooltip, Grid
} from '@mui/material';
import { useProjectContext } from '../contexts/ProjectContext';
import { getProjectMetrics, createIgnore } from '../services/api';
import { BarChart as BarChartIcon, Block as IgnoreIcon, FolderSpecial as DirectoryIcon } from '@mui/icons-material';
...
const MetricCard = ({ title, value, color = "primary", icon }) => (
    <Card elevation={2}>
        <CardContent sx={{ textAlign: 'center' }}>
            {icon}
            <Typography variant="h4" sx={{ color: `${color}.main`, fontWeight: 'bold' }}>{value}</Typography>
            <Typography variant="body2" color="text.secondary">{title}</Typography>
        </CardContent>
    </Card>
);

const ProjectMetricsTable = ({ projectData, onIgnoreFile }) => {
                    ...
                    <TableBody>
                        {sortedFiles.map(file => (
                            <TableRow key={file.path}>
                                <TableCell sx={{ fontFamily: 'monospace', fontSize: '0.8rem' }}>{file.path}</TableCell>
                                <TableCell>
                                    <Chip
                                        label={file.lines}
                                        size="small"
                                        sx={{ backgroundColor: getLineColor(file.lines), color: 'white' }}
                                    />
                                </TableCell>
                                <TableCell>{file.tokens}</TableCell>
                                <TableCell>
                                    <Tooltip title="Adicionar à lista de ignorados">
                                        <IconButton size="small" onClick={() => onIgnoreFile(file.path, projectData.projectId)}>
...
const MetricsPage = ({ showNotification }) => {
    ...
    const handleIgnoreFile = async (filePath, projectId) => {
        ...
        try {
            ...
            showNotification(`'${fileName}' adicionado à lista de ignorados do projeto.`, 'success');
            fetchMetrics();
        } catch (error) {
    ...
    const handleIgnoreDirectory = async (fullDirectoryPath) => {
        ...
        const project = metrics?.projects.find(p => p.projectTitle === projectTitle);
        if (!project) {
            showNotification(`Projeto '${projectTitle}' não encontrado para associar a regra.`, 'error');
            return;
        }
        ...
        if (directoryToIgnore === '.') {
            showNotification('Não é possível ignorar o diretório raiz a partir desta visualização.', 'warning');
            return;
        }

        if (!window.confirm(`Tem certeza que deseja ignorar o diretório '${directoryToIgnore}' no projeto '${project.projectTitle}'?`)) {
            return;
        }

        try {
            await createIgnore({ path: directoryToIgnore, scope: 'project', projectId: project.projectId });
            showNotification(`'${directoryToIgnore}' adicionado à lista de ignorados do projeto '${project.projectTitle}'.`, 'success');
            fetchMetrics();
        } catch (error) {
        ...
        <Container maxWidth="lg">
            <Box sx={{ mb: 4 }}>
                ...
                <Typography variant="body1" color="text.secondary" paragraph>
                    Analise a complexidade dos seus projetos através de métricas de linhas de código, estimativa de tokens e distribuição de arquivos por diretório.
                </Typography>
            </Box>
                ...
                <Box>
                    <Grid container spacing={2}>
                        <Grid item xs={12} md={6}>
                            <MetricCard title="Estimativa Total de Tokens" value={metrics.totalTokens} icon={<BarChartIcon color="primary" sx={{ fontSize: 40 }} />} />
                        </Grid>
                        <Grid item xs={12} md={6}>
                            <MetricCard title="Diretórios Analisados" value={metrics.directoryMetrics.length} color="secondary" icon={<DirectoryIcon color="secondary" sx={{ fontSize: 40 }} />} />
                        </Grid>
                    </Grid>
                    ...
                    <Divider sx={{ my: 3 }}><Chip label="Métricas por Arquivo" /></Divider>
                    
                    {metrics.projects.map(projectData => (
                        <ProjectMetricsTable key={projectData.projectId} projectData={projectData} onIgnoreFile={handleIgnoreFile} />
                    ))}
                </Box>
            ) : (
